...
// There's a remaining duplicate that didn't get caught by the regex
content = content.replace(/<div className="w-full mt-4">\s*<button onClick=\{\(\) => setIsEditingTime\(!isEditingTime\)\} className="w-full bg-surface hover:bg-primary\/5 text-primary py-3 rounded-md font-sans text-sm font-medium transition-colors flex items-center justify-center gap-2 active-sink halo-border">\s*<Clock size=\{16\} \/> Reschedule Time\s*<\/button>\s*\{isEditingTime && \([\s\S]*?<\/button>\s*<\/div>\s*\)\}\s*<\/div>/g, '');
